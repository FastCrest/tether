...
from tether.eval.episode_evidence import validate_episode_evidence
...
def retrieve(
    ...
    *,
    ...
    runner([modal_binary, "volume", "get", VOLUME, remote, str(destination)], check=True)
    files = [path for path in destination.rglob("*") if path.is_file()]
    if sum(path.stat().st_size for path in files) > max_bytes:
        raise ValueError("retrieved evidence exceeds the configured byte limit")
...
def main() -> None:
    ...
    parser.add_argument("--modal-binary", default="modal")
