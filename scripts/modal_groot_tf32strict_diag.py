"""Modal A100 DIAGNOSTIC (M4 #53, not a qualification run).

Interventional test for the CUDA-only 1e-3 gap: runs the UNMODIFIED
receipt-grade harness with `NVIDIA_TF32_OVERRIDE=0` in its environment
(strict IEEE fp32 in cuBLAS/cuDNN) and writes the outcome to a
DIAGNOSTIC path (`groot-tf32strict-diag.json`), never the parity receipt
path. If the gap collapses to ~1e-6, the root cause is backend matmul
precision (cublas default math), not export fidelity. If it persists,
something else contributes. Gates, harness, thresholds untouched.
...
Usage:
    modal run scripts/modal_groot_tf32strict_diag.py --reuse-export
"""
...
app = modal.App("tether-groot-tf32strict-diag")
...
def run_export_parity(
    ...
    receipt_path = Path(PARITY_OUT_PATH) / "groot" / "groot-tf32strict-diag.json"
    cmd = [
        sys.executable,
    ...
    # Interventional diagnostic only: force strict fp32 math in the CUDA
    # libraries for the harness subprocess. See module docstring.
    harness_env["NVIDIA_TF32_OVERRIDE"] = "0"
    proc = subprocess.run(
