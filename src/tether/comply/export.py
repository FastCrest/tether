...
from tether.comply.collect import collect_evidence, copy_source_artifacts
from tether.comply.mapping import build_gap_report, build_regulatory_mapping
from tether.comply.pdf import write_text_pdf
from tether.comply.sbom import generate_sbom, write_sbom
from tether.comply.schemas import DeploymentMetadata, SCHEMA_VERSION, utc_now_iso
from tether.comply.signing import sign_payload, verify_payload_signature
...
def _render_status(status: str) -> str:
    if status == "covered_by_tether_evidence":
        return "Covered by Tether evidence"
...
def _render_technical_file(
    *,
    ...
    lines = [
        "# Tether EU Technical Documentation Evidence File",
        "",
        f"Generated: {generated_at}",
        "",
        "## Scope",
        "",
        "This file is an evidence pack for a robot deployment running Tether. Tether does not declare CE conformity. The manufacturer remains responsible for the system-level risk assessment, intended-purpose documentation, conformity assessment route, and notified-body interactions.",
        "",
        "## Deployment",
        "",
        ...
        "## Regulatory Mapping",
        "",
        "| Control | Regulation | Status | Tether evidence | Customer action |",
        "|---|---|---|---|---|",
    ]
    for control in mapping:
        lines.append(
            "| {control_id} | {regulation} {article} | {status} | {evidence} | {gap} |".format(
                ...
                evidence=", ".join(control["tether_evidence"]),
    ...
    lines.extend([
        ...
        "",
        "Run `tether comply verify-bundle <bundle_dir>` to check the conformity JSON signature, parity certificate signature, and artifact hashes.",
        "",
    ])
    return "\n".join(lines)
...
def _render_gap_report(gaps: list[dict[str, Any]]) -> str:
    lines = [
        "# Tether Comply Gap Report",
        "",
        "Tether evidence reduces the compliance-documentation workload, but it does not replace the manufacturer's legal, safety, and quality-system obligations.",
        "",
    ]
    if not gaps:
        lines.append("No open gaps detected by Tether Comply. A qualified reviewer should still inspect the full technical file.")
        return "\n".join(lines) + "\n"
    for gap in gaps:
        lines.extend([
            f"## {gap['control_id']}",
...
def _render_trust_page(deployment: DeploymentMetadata, evidence: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# Tether Deployment Trust Page",
        "",
        "## Data Flow",
        "",
        "Robot -> Tether runtime -> on-device ActionGuard -> local/customer-controlled audit log -> exported conformity bundle.",
        "",
        "## Stored Data",
        "",
        "- Model/config hashes",
        ...
        "## Encryption",
        "",
        "Tether writes local files. The deployment owner is responsible for disk encryption, access control, and backup policy in the target environment.",
        "",
    ])
...
def _render_security_whitepaper(deployment: DeploymentMetadata) -> str:
    return "\n".join([
        "# Tether Security Whitepaper",
        "",
        "## Secure-by-Design Position",
        "",
        "Tether minimizes cloud dependency by running model serving, ActionGuard, and audit logging on the robot or customer-controlled infrastructure.",
        "",
        "## Integrity Controls",
        "",
...
def _render_vulnerability_manifest(deployment: DeploymentMetadata) -> str:
    return "\n".join([
        ...
        "## Update Handling",
        "",
        "When a model, runtime, or safety config changes, rerun `tether verify` and `tether comply export` so the technical file contains the updated parity certificate and artifact hashes.",
        "",
    ])
...
def export_conformity_bundle(
    *,
    ...
    write_text_pdf(out / "TECHNICAL_FILE.pdf", title="Tether EU Technical Documentation Evidence File", text=technical_md)
    ...
    conformity: dict[str, Any] = {
        ...
        "legal_notice": "Tether Comply produces evidence for a manufacturer's technical file; it does not certify or declare conformity.",
        "evidence": evidence,
