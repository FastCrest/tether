"""Regulatory-control mapping for Tether Comply bundles."""

from __future__ import annotations

from typing import Any

from tether.comply.schemas import RegulatoryControl


def _status(condition: bool, *, customer_gap: str = "") -> str:
    if condition and not customer_gap:
        return "covered_by_tether_evidence"
    if condition:
...
def build_regulatory_mapping(evidence: dict[str, Any], *, sbom_present: bool) -> list[RegulatoryControl]:
    ...
    return [
        RegulatoryControl(
            ...
            tether_evidence=["parity.cert.json", "PARITY.md", "conformity.json"],
        ...
        RegulatoryControl(
            ...
            tether_evidence=["ActionGuard config", "safety-violation log", "webhook/event counters when enabled"],
        ...
        RegulatoryControl(
            ...
            tether_evidence=["signed parity cert", "ActionGuard limits", "SBOM", "audit error/safety counters"],
        ...
        RegulatoryControl(
            ...
            tether_evidence=["ActionGuard as deterministic safety boundary", "parity cert for deployed model hash", "re-certify-on-update evidence"],
        ...
        RegulatoryControl(
            ...
            tether_evidence=["SBOM.cyclonedx.json", "VULNERABILITY_HANDLING.md"],
        ...
        RegulatoryControl(
            ...
            tether_evidence=["edge image hashing/redaction", "instruction hashing where enabled", "TRUST_PAGE.md deletion/retention section"],
        ...
        RegulatoryControl(
            control_id="tether.parity.identity",
            regulation="Tether deployment control",
            ...
            tether_evidence=["parity.cert.json signature", "model hash", "config hash"],
...
def build_gap_report(mapping: list[RegulatoryControl]) -> list[dict[str, Any]]:
    ...
    for control in mapping:
        if control.status == "covered_by_tether_evidence":
            continue
        gaps.append({
