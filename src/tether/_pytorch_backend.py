"""PyTorch reference backend for ``tether validate`` round-trip parity.

Reference semantics (v1)
------------------------
The "PyTorch reference" used by :mod:`tether.validate_roundtrip` is **not** the
...
``tether export`` uses to materialize the action-expert / DiT graph for ONNX
...
* ``smolvla`` → :func:`tether.exporters.smolvla.build_expert_stack`
* ``pi0``     → :func:`tether.exporters.pi0.build_pi0_expert_stack`
* ``gr00t``   → :func:`tether.exporters.gr00t.build_gr00t_full_stack`
...
def load_pytorch_backend(
    ...
    Reads ``tether_config.json`` from ``export_dir`` to determine the model
    type and (optionally) the denoise step count, downloads / loads the
    checkpoint via :func:`tether.checkpoint.load_checkpoint`, and reconstructs
    the same decomposed stack the exporter would build.

    Parameters
    ----------
    export_dir:
        Directory produced by ``tether export`` containing
        ``tether_config.json``.
    model_id:
        Optional HuggingFace ID for the upstream checkpoint. If ``None``, the
        ``model_id`` field from ``tether_config.json`` is used.
    device:
...
class PyTorchBackend:
    ...
    Holds a reference to the decomposed expert stack plus the ``tether_config``
    ...
    position ids (matching what ``tether export`` actually emits). The
    ...
    def forward(
        ...
        # Canonical scheme used by tether.inference.flow_matching_denoise +
