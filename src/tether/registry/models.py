...
implement the predicates `tether models list --family X --device Y` consume.
...
- `requires_export`: True if customer must run `tether export` after pull (raw weights);
                     False if the repo already contains a Tether-ready export (vlm_prefix.onnx etc.).
...
class ModelEntry:
    ...
    def resolved_vla_type(self) -> str:
        ...
        Added lift #1 Day 10 for ``tether models list`` + ``tether models info``
        + ``tether export <model_id>`` dispatch.
        """
...
# Lazy import — keeps `import tether.registry` cheap if data.py grows large
def _load_registry() -> list[ModelEntry]:
    from tether.registry.data import REGISTRY as _R
    return list(_R)
