...
`tether serve <export_dir>` and get the same API regardless of model.
...
class Pi0OnnxServer:
    ...
    # TetherServer has these methods; we mirror them so create_app's lifespan
    ...
    async def start_batch_worker(self) -> None:
        # Monolithic runtime is single-request; batching is TetherServer's job.
        return None
