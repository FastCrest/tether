...
`tether go --model pi05 --embodiment franka` calls into here. The resolver
consults the in-package registry (`tether.registry.REGISTRY`) and applies
preference rules:
...
3. If the chosen entry has `requires_export=True`, surface that in the
   ...
   tell the user to run `tether models export` first.

Resolver is pure-Python (no I/O); the calling layer in `tether go` is what
actually pulls + serves.
"""
from __future__ import annotations

from dataclasses import dataclass
from tether.registry import ModelEntry, REGISTRY, by_id
...
def _unsupported_device_error(
    ...
    suggestion = (
        ...
        f" Run `tether models list --device {device_class}` to browse supported models."
    )
...
def resolve_model(
    ...
    if family_only:
        ...
        return ResolveResult(
            ...
            notes=[
                ...
                f"consider explicit --device-class or `tether models list` to browse."
            ],
        )
    ...
    raise ModelResolverError(
        ...
        f"  Try: tether models list --device {device_class}"
    )
