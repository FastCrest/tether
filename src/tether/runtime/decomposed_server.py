"""Pi05DecomposedServer -- wraps Pi05DecomposedInference behind the tether
serve interface.

Per ADR 2026-04-25-decomposed-dispatch-via-tether-serve: closes the
B.4/B.5 measurement gap. TetherServer's create_app router falls through
to the legacy TetherServer for `export_kind=decomposed` exports, which
looks for `expert_stack.onnx` and fails with "No ONNX model found".

This wrapper lets the router add a 4th branch:
    if tether_config.export_kind == "decomposed":
        server = Pi05DecomposedServer(...)

The class exposes the TetherServer interface (predict_from_base64_async,
...
# tether_config.json `camera_resolution` field; we read it at load time.
DEFAULT_CAMERA_RESOLUTION = 224

# Default chunk size + max action dim for pi05 (matches the export). Read
# from tether_config.json at load time; these are the conservative defaults
...
class Pi05DecomposedServer:
    ...
    Mirrors the TetherServer attributes that create_app + the /act handler
    ...
    def __init__(
        ...
        # TetherServer-interface mirror attributes (populated during load
        ...
        # Loaded from tether_config.json at .load() time.
    ...
    def _inference_mode(self) -> str:
        """Mirrors TetherServer._inference_mode for /act response building.
    ...
    def load(self) -> None:
        ...
        from tether.runtime.pi05_decomposed_server import (
    ...
    def _load_normalizer_stats(self) -> None:
        ...
        """
        from tether.runtime.adapters.vla_eval import load_normalizer_stats
    ...
    def _infer_teacher_ref(self) -> str | None:
        ...
        # common case at this stage). Safe because all Tether pi05 distills
    ...
    def predict_from_base64(
        ...
        """Sync entry point. Mirrors TetherServer.predict_from_base64."""
    ...
    async def predict_from_base64_async(
        ...
        """Async entry point matching TetherServer's contract."""
        ...
        *,
        ...
        try:
            ...
            if self._bid_config is not None:
                ...
                from tether.correction.bid import predict_chunk_bid
    ...
    def _tokenize(self, instruction: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        in src/tether/distill/pi05_state_out_processor.py). Without this
