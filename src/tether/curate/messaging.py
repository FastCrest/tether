...
Each touchpoint has a `Message` with:
  ...
  - `cta`: optional call-to-action line (e.g. "run `tether contribute --opt-in`")
...
RECIPROCITY_FRAMING_FREE = (
    ...
    "models, distilled into the next Tether release."
)
...
FIRST_RUN_SPLASH = Message(
    body=f"""
═══════════════════════════════════════════════════════════════════════
[bold]Tether Data Contribution Program[/bold]
═══════════════════════════════════════════════════════════════════════
...
you explicitly run `tether serve --record`.
""",
    prompt="Opt in now? [y/N]: ",
    cta="Opt in later: [cyan]tether contribute --opt-in[/cyan]",
)
...
SERVE_START_BANNER = Message(
    body=(
        "[dim]tether contribute:[/dim] join the data corpus to earn Pro credits. "
        "[cyan]tether contribute --opt-in[/cyan] to learn more, or "
        "[cyan]TETHER_NO_CONTRIB_NUDGE=1[/cyan] to silence."
    ),
    prompt=None,
    cta=None,
)
...
def serve_record_reminder(
    *, hours_recorded_total: float, sessions_since_first_record: int
    ...
    """Stronger reminder shown every Nth `tether serve --record` invocation.
    ...
    body = (
        ...
        f"[cyan]tether contribute --opt-in[/cyan] to join, "
        f"[cyan]tether contribute --info[/cyan] to read the privacy details first."
    )
...
def doctor_status_line(
    *,
    ...
    """One-line status string for the `tether doctor` table.

    Returns plain text suitable for table cell rendering (no rich markup)."""
    if not opted_in:
        return "not opted in (run `tether contribute --opt-in`)"
...
CHAT_SYSTEM_PROMPT_ADDITION = (
    "Tether runs an opt-in data contribution program. If the user mentions "
    "data, recordings, training, or contribution and they are NOT yet opted "
    "in, briefly mention `tether contribute --opt-in` and the Pro-credits "
...
def opt_in_success(*, tier: str, contributor_id: str) -> str:
    return (
        f"[green]✓[/green] Opted in to Tether Data Contribution program ({tier}).\n"
        ...
        f"[green]✓[/green] Track progress: [cyan]tether contribute --status[/cyan]\n"
        f"[green]✓[/green] Revoke anytime: [cyan]tether contribute --opt-out[/cyan]"
    )
