"""`tether contribute` CLI subcommand.
...
Surface:
  ...
  tether contribute --purge ID       → drop a specific queued episode before upload
...
from tether.curate import messaging

logger = logging.getLogger(__name__)

contribute_app = typer.Typer(
    name="contribute",
    help="Tether Data Contribution — opt in, opt out, check status, or revoke.",
...
def main(
    ...
    """Tether Data Contribution program — opt in, opt out, or check status."""
...
def _cmd_info() -> None:
    ...
    body = f"""
[bold]Tether Data Contribution — what you're agreeing to[/bold]
...
- Robot recordings made with [cyan]tether serve --record[/cyan]
...
- Anything you mark sensitive via [cyan]tether inspect traces --mark-sensitive ID[/cyan]
...
def _cmd_status() -> None:
    if not curate_consent.is_opted_in():
        console.print(
            ...
            "[dim]Run [cyan]tether contribute --opt-in[/cyan] to join, or "
            "[cyan]tether contribute --info[/cyan] for privacy details first.[/dim]"
        )
        return
    ...
    if stats is None:
        console.print(
            ...
            "unavailable. Local queue: [cyan]tether contribute --inspect[/cyan].[/dim]"
        )
        return
    if stats.get("error") == "not_found":
        console.print(
            ...
            "[cyan]tether contribute --inspect[/cyan].[/dim]"
        )
        return
...
def _cmd_inspect() -> None:
    ...
    if files_q:
        ...
        console.print(
            ...
            "[cyan]tether contribute --purge <name>[/cyan][/dim]"
        )
    else:
...
def _cmd_purge(*, target: str, yes: bool) -> None:
    ...
    from tether.curate.uploader import DEFAULT_QUEUE_DIR
    ...
    if not target_path.exists() or not target_path.is_file():
        console.print(
            ...
            f"{queue}.\n"
            f"[dim]Run [cyan]tether contribute --inspect[/cyan] to see queued files.[/dim]"
        )
        raise typer.Exit(2)
...
def _cmd_revoke_status(*, request_id: str) -> None:
    ...
    if overall != "completed":
        console.print(
            ...
            f"Re-run [cyan]tether contribute --revoke-status {request_id}[/cyan] "
            f"to refresh.[/dim]"
        )
