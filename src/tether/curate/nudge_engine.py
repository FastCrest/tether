...
and the kill switch (`TETHER_NO_CONTRIB_NUDGE=1`) gives a hard exit.

State lives at `~/.tether/curate_nudge.json`:
...
- serve_start   → every serve start UNTIL decided OR TETHER_NO_CONTRIB_NUDGE=1
...
DEFAULT_NUDGE_STATE_PATH = "~/.tether/curate_nudge.json"
SERVE_RECORD_REMINDER_EVERY_N = 10
KILL_SWITCH_ENV = "TETHER_NO_CONTRIB_NUDGE"
...
def doctor_status(*, hours_contributed: float | None = None) -> str:
    """One-line passive status for the `tether doctor` table. Never gated by
