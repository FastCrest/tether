...
- Free tier: opt-in. Receipt at ~/.tether/consent.json.
...
This module is intentionally separate from `tether.pro.consent.ProConsent`
...
CONSENT_VERSION = 1
DEFAULT_CONSENT_PATH = "~/.tether/consent.json"
...
@dataclass(frozen=True)
class CurateConsent:
    """Persisted consent receipt at `~/.tether/consent.json`.
...
def derive_contributor_id() -> str:
    ...
    salt = b"tether-curate:contributor:v1"
...
def load(path: str | Path = DEFAULT_CONSENT_PATH) -> CurateConsent:
    ...
    if not path_obj.exists():
        raise ConsentNotFound(
            ...
            f"Run `tether contribute --opt-in` to create one."
        )
    try:
        ...
        raise ConsentCorrupted(
            ...
            f"Run `tether contribute --opt-out` to clear it, then "
            f"`tether contribute --opt-in` to re-create."
        ) from exc
    if receipt.version != CONSENT_VERSION:
        raise ConsentCorrupted(
            f"consent_version drift: receipt={receipt.version}, "
            f"current={CONSENT_VERSION}. Run `tether contribute --opt-out` then "
            f"`tether contribute --opt-in` to re-create."
        )
    return receipt
...
def revoke(path: str | Path = DEFAULT_CONSENT_PATH) -> bool:
    ...
    `tether contribute --revoke` (which calls this AND POSTs to the worker).
    """
    path_obj = Path(path).expanduser()
    if not path_obj.exists():
        return False
