    ...
    *,
    ...
    if processor_source:
        ...
        if processor_path.exists():
            ...
            processor_identity = (
                "sha256:"
                + hashlib.sha256(json.dumps(processor_files, sort_keys=True).encode()).hexdigest()
            )
            ...
            if "/" not in resolved_processor:
                raise CheckpointError(
                    "A processor checkpoint must be a local directory or Hugging Face repository."
                )
            if not processor_revision:
                raise CheckpointError(
                    "A remote processor checkpoint requires --processor-revision."
                )
    ...
    if path.exists():
        ...
        if kind != "auto" and kind != detected:
            raise CheckpointError(
                f"Checkpoint kind {kind!r} does not match detected kind {detected!r}."
            )
        resolved_base = base
        if detected == "smolvla-lora" and not resolved_base:
            try:
                resolved_base = json.loads(adapter_config.read_text()).get(
        ...
        if (
            detected == "smolvla-lora"
            and resolved_base
            and not Path(resolved_base).expanduser().is_dir()
            and not base_revision
        ):
            raise CheckpointError(
                "A remote LoRA base requires --adapter-base-revision for reproducible evidence."
            )
    ...
    if kind == "smolvla-lora":
        if not base:
            raise CheckpointError(
                "A remote SmolVLA LoRA adapter requires an explicit base checkpoint."
            )
    ...
    if "/" not in remote:
        raise CheckpointError(
            f"Checkpoint path does not exist and is not a Hugging Face repository: {raw}"
        )
    if not revision:
        raise CheckpointError(
            "A Hugging Face checkpoint requires --checkpoint-revision for reproducible evidence."
        )
    identity = (
        f"hf-lora:{remote}@{revision}+{base}@{base_revision}"
        if detected == "smolvla-lora"
        else f"hf:{remote}@{revision}"
    )
    return CheckpointSpec(
        detected,
        remote,
