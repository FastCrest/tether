...
vla_type, tether_version, torch_version)`` — any of these changing
invalidates the cache. The cache file lives at
``~/.cache/tether/inference_weights/<hash>.pt`` and stores the flat
dict via ``torch.save``.
...
- ``vla_type``: composition class name (``Pi05VLA`` / ``GR00TVLA``).
  ...
  differs.
- ``tether_version``: refactors to the spine slot layout (e.g. a
...
def _cache_dir() -> Path:
    """Returns the ~/.cache/tether/inference_weights/ directory (created on demand)."""
    base = Path.home() / ".cache" / "tether" / "inference_weights"
...
def _model_hash(
    ...
    `tether_version` and `torch_version` default to the current process'
    values when not supplied — callers can pin them for tests.
    """
    if tether_version is None:
        from tether import __version__ as tether_version  # noqa: F811
    ...
    tup = (
        ...
        f"vla_type={vla_type}|"
        f"tether_version={tether_version}|"
...
def load_or_build(
    ...
    *,
    ...
    Args:
        ...
        tether_version: Override tether version (for tests).
