...
from tether.chat.loop import LoopState, SYSTEM_PROMPT, build_system_prompt
...
if TEXTUAL_AVAILABLE:
    ...
    class ChatTUI(App):
        """4-panel Textual chat for the tether agent."""
        ...
        async def _one_turn(self, tools: list[dict[str, Any]] | None) -> dict[str, Any]:
            ...
            from tether.chat.backends import assemble_stream
...
def run_tui(proxy_url: str | None = None, dry_run: bool = False) -> None:
    ...
    textual isn't installed (so `tether chat --tui` is safe even without
    the [tui] extra)."""
