...
The legacy module ``tether.exporters.gr00t`` stays available for
...
from tether.decompose import DecomposedRMSNorm, DecomposedRotaryEmbedding  # noqa: F401  (used by classes below)
...
__all__ = [
    "export_gr00t",
    "export_gr00t_full",
    # Moved here from the deleted ``gr00t_exporter.py`` at Day 11 sunset:
    "GR00T_BLOCK_PREFIX",
    "GR00T_META_KEYS",
    "GR00TDiTBlock",
    "GR00TExpertStack",
    ...
    "GR00TActionDecoder",
    "GR00TFullStack",
    "build_gr00t_expert_stack",
    "build_gr00t_full_stack",
]


# ════════════════════════════════════════════════════════════════════════
# Below: load-bearing GR00T DiT internals moved from the deleted
# ``src/tether/exporters/gr00t_exporter.py`` at Day 11 sunset. Behavior is
...
class GR00TStateEncoder(nn.Module):
    ...
    action head is conditioned on the current pose. Prior tether exports
...
def build_gr00t_full_stack(
    ...
    # State encoder (new 2026-04-19; absent from prior tether exports).
