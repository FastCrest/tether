...
FluxVLA's vendored Triton kernels (in ``src/tether/kernels/triton/``) hard-code
...
A customer attempting ``tether serve --fast-kernels`` on a non-PaliGemma SigLIP
...
is **refuse-to-fast-kernels**: validate the model's `tether_config.json` against
...
# Required keys in `tether_config.json` (or the kwargs passed to the runtime)
...
def validate_shape_signature(
    tether_config: dict[str, Any],
) -> tuple[bool, str]:
    """Validate a model's shape signature against the Pi0.5 V1 whitelist.

    Args:
        tether_config: Parsed ``tether_config.json`` (or any dict with the
    ...
    """
    missing = [k for k in REQUIRED_CONFIG_KEYS if k not in tether_config]
    if missing:
        return (
            False,
            f"tether_config missing required keys for fast-kernels: {sorted(missing)}. "
            f"--fast-kernels requires PaliGemma SigLIP-base shapes; fall back to ORT "
            f"or supply a richer tether_config.json.",
        )

    sig = PI05_SHAPE_SIGNATURES["paligemma_siglip_base"]
    mismatches: list[str] = []
    for key in REQUIRED_CONFIG_KEYS:
        got = tether_config[key]
        want = sig[key]
        if got != want:
    ...
    if "num_patches" in tether_config:
        derived = (tether_config["image_size"] // tether_config["patch_size"]) ** 2
        if tether_config["num_patches"] != derived:
            mismatches.append(
                f"num_patches: got={tether_config['num_patches']!r}, "
